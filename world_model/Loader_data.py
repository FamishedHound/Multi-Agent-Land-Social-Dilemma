...
def train_gan():
    ...
    optimizer =Adam(model.parameters(), lr=3e-4) #SGD(model.parameters(), lr=0.01, momentum=0.9)
    ...
    for epoch in range(30):
        ...
        for i, img in enumerate(data_train_loader):
            ...
            img_sharp = img_sharp.float().squeeze(1).cuda()
            img_sharp = img_sharp[:,0,:,:].unsqueeze(1)
            img_blur = img_blur.float().squeeze(1).cuda()
            img_blur = img_blur[:,0,:,:].unsqueeze(1)
        ...
        if epoch%2==0:
            predicted_output_img = img_deblur[0][0].detach().cpu().numpy().squeeze()
            actual_output = img_sharp[0][0].detach().cpu().numpy().squeeze()
            print(predicted_output_img)
            print(actual_output)
        # plt.plot(plot)
        # plt.show()
        # if epoch % 2 == 0:
...
if __name__ == '__main__':
    bla = 1
    data_path = "C:\\Users\\LukePC\\PycharmProjects\\polinators_social_dilema\\train"
